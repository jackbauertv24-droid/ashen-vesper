...
- Run `npm test` and `npm run validate:assets` for submissions; `npm run test:stage` when gallery/preview/study changes; `npm run test:route` for gameplay changes and required regression checks. New controls/behavior need meaningful focused tests and actual input evidence. Passing the old encounter does not prove a new map is integrated.
