# Counterweight Works — layout baseline contribution

Use this as the geometry starting point, then make any adjustments in a recorded v002 deviation. [Layout JSON](05-counterweight-works.json) · [neutral diagram](05-counterweight-works.svg) · [scrolling preview](preview.html) · [shared workflow](../README.md).

**Bounds 9216 × 2640; 6 rooms, 5 joins.** This example takes priority over the earlier freeform envelope when submitting a baseline-layout PR. It is not the current playable Pilgrim Road or a finished stage.

## Written art direction

Counterweight service works with stacked limestone maintenance galleries, aged timber decks, dark iron ratchets, separate bronze weights/chains and narrow cold-sky vents. Shared corridors use one consistent physical scene.
...
| --- | --- | --- | --- | --- |
| 05-A | (3072, 1960) | x=0..2048, y=0..640 | 5 | 5 |
| 05-B | (3072, 1320) | x=0..5120, y=0..640 | 19 | 5 |
| 05-C | (4096, 1320) | x=0..5120, y=0..640 | 17 | 5 |
| 05-D | (4096, 680) | x=0..5120, y=0..640 | 22 | 11 |
| 05-E | (0, 680) | x=0..4096, y=0..640 | 17 | 3 |
| 05-F | (0, 40) | x=0..4096, y=0..640 | 8 | 1 |
...
| --- | --- | --- | --- | --- |
| 05-join-1 | 05-A → 05-B | stair-band-transition / seam | (392, 0) | (392, 640) |
| 05-join-2 | 05-B → 05-C | doorway / seam | (5120, 512) | (4096, 512) |
| 05-join-3 | 05-C → 05-D | stair-band-transition / seam | (4352, 0) | (4352, 640) |
| 05-join-4 | 05-D → 05-E | doorway / seam | (0, 512) | (4096, 512) |
| 05-join-5 | 05-E → 05-F | stair-band-transition / seam | (3712, 0) | (3712, 640) |

Shared region **05-B / 05-C**: stage rectangle (4096,1320,4096,640). Owner 05-B owns collision and persistent items in that overlap. Merge the scene; never separate these rooms to hide overlap.
