...
function draw(){if(!heroArt)return;ctx.fillStyle="#0b131b";ctx.fillRect(0,0,1280,720);const fog=ctx.createLinearGradient(0,0,0,720);fog.addColorStop(0,"#18242d");fog.addColorStop(1,"#081017");ctx.fillStyle=fog;ctx.fillRect(0,0,1280,720);ctx.save();ctx.translate(-s.camera*.18,-s.cameraY*.18);ctx.globalAlpha=.42;for(let x=-40;x<5200;x+=320)ctx.drawImage(art.arch,119,41,786,945,x,90,430,520);ctx.restore();ctx.save();ctx.translate(-s.camera,-s.cameraY);for(const p of platforms)platform(p);drawLever();drawPortcullis(ctx,{x:LEVEL.gate,floorY:600,open:s.leverOn,grate:art.grate,stone:art.stone});if(!s.vialTaken)ctx.drawImage(art.vial,1868,520+Math.sin(s.time*3)*5,64,64);enemy();drawHero(ctx,s,heroArt);ctx.fillStyle="#d9bc82";ctx.font="17px Georgia";for(const[x,t]of[[180,"I · BROKEN ARCADE"],[1280,"II · VIAL LEDGE"],[2140,"III · LEVER COURT"],[3220,"IV · GATE WALK"]])ctx.fillText(t,x,220);ctx.fillText("E · PULL",LEVEL.lever-35,475);ctx.restore();$("#health").textContent="♥".repeat(s.hp)+"♡".repeat(5-s.hp);$("#progress").value=s.x/LEVEL.width*100;$("#notice").textContent=s.noticeTime?s.notice:"";$("#status").textContent=s.complete?"STAGE BLOCKOUT COMPLETE":s.x<1100?"BROKEN ARCADE":s.x<2100?"VIAL LEDGE":s.x<3000?"LEVER COURT":"GATE WALK"}
